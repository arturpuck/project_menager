<x-base title="{{$title}}" description="{{$description}}">
      ...
      @csrf
         <div class="filter-controls">
            ...
            <labeled-select v-model="filterTaskId" name="task" 
               v-bind:displayed-values="{{json_encode($tasks->pluck('name_in_current_language'),true)}}">
               {{__('work_range')}} : 
            </labeled-select>
            <labeled-select v-model="filterStatusId" name="status_id"
               v-bind:values="{{json_encode($projectStatuses->pluck('id'), true)}}"
               v-bind:displayed-values="{{json_encode($projectStatuses->pluck('name_in_current_language'), true)}}">
               {{__('project_status')}} : 
            </labeled-select>
      ...
      <table class="table">
         ...
         <tbody>
           <tr class="table-row" v-for="project in matchingProjects">
               ...
               <td v-text="project['status']['name_in_current_language']" v-bind:class="{'finished-status' : checkProjectStatus(project)}" class="table-cell status-cell"></td>
   ...
   <project-form 
      ...
      v-bind:tasks-names="{{json_encode($tasks->pluck('name_in_current_language'),true)}}"
      ...
      v-bind:work-range-options="{{json_encode($tasks->pluck('name_in_current_language'), true)}}"
      ...
      v-bind:payment-statuses-values="{{json_encode($paymentStatuses->pluck('name_in_current_language'), true)}}"
      v-bind:project-statuses-ids="{{json_encode($projectStatuses->pluck('id'), true)}}"
      v-bind:project-statuses-values="{{json_encode($projectStatuses->pluck('name_in_current_language'), true)}}"
